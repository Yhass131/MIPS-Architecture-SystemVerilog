...
### ALU
Capable of 12 operations on its inputs but which operation runs is dictated by the ALU control, shown above. It also calculates the three output flags regardless of the instruction which isn't the usual case but isn't a consern for the scope of this project. 
...
The jump target address logic incudes a MUX that choses between the calculated address or an address form a register for the case of `jr` that followes `jal`. 